...
components/
  ...
  dialogs/           完成及提前结束对话框
  layout/            顶部导航和页面容器
hooks/               倒计时、番茄状态、背景设置/图片、localStorage、全屏 Hook
lib/                 时间、番茄状态机、背景预设/存储/IndexedDB、会话和统计纯函数
types/               计时器、番茄阶段、记录、设置与背景严格类型定义
styles/              翻页时钟样式
```
...
计时器存储结构当前为 V2，新增 `mode` 和 `pomodoro` 阶段状态。读取 V1 状态时默认迁移为自由专注；设置缺少番茄字段时自动补入 25/5/15 分钟和 4 轮默认值；旧历史记录缺少模式字段时仍按自由专注显示。
