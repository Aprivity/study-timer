...
- “更多”扩展能力入口，以及 AI 计划图与最近 7 天 AI 历史专注分析
...
AI 历史专注分析位于 `更多 → AI 历史专注分析`，固定查看最近 7 个本地自然日。前端先从本地历史中确定性计算总专注时间、专注次数、平均单次时长、按日汇总和按任务汇总；关键数字会立即显示，不等待 AI。
...
后端会再次校验两个汇总口径一致，并由程序计算最终统计和前三个主要任务。AI 只接收经过校验的统计包，返回简短总体总结、1～3 条专注规律和 1～3 条改进建议。页面不提供聊天输入、评分、雷达图或复杂 Dashboard，也不会修改历史、创建任务或启动计时器。

逐条历史记录、精确开始/结束时间、分类和计时器状态不会发送到该接口；最近 7 天没有正专注时长时不会请求 AI。
...
核心计时、专注历史、热力图、设置和备份都保存在当前浏览器中，不会自动同步原始记录。只有使用 AI 填写或 AI 计划图时，对应自然语言才会发送到 AI Backend；进入 AI 历史专注分析页时只发送最近 7 天的按日和按任务汇总，不发送逐条记录或精确时间。AI 计划图以 PNG Blob 返回浏览器用于预览和下载，V1 不提供前端图片历史或云端图库。
...
AI 测试覆盖统一自然语言计时输入、自由专注/番茄配置应用时机、计划图请求与 Blob 生命周期，以及 7 天历史聚合、接口一致性校验、空历史、分析加载和错误重试。
