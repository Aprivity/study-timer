...
export function ConfirmEndDialog({ open, onSave, onDiscard, onContinue }: { open: boolean; onSave: () => void; onDiscard: () => void; onContinue: () => void }) {
  const dialogRef = useDialogFocus(open, onContinue);
  if (!open) return null;
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(e) => { if (e.currentTarget === e.target) onContinue(); }}>
    <div ref={dialogRef} className="dialog" role="dialog" aria-modal="true" aria-labelledby="end-title" aria-describedby="end-description">
      ...
      <p id="end-description">你可以保留已经投入的时间，也可以放弃本次记录。</p>
