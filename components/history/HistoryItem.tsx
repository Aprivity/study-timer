...
export function HistoryItem({ session, onDelete }: { session: FocusSession; onDelete: (id: string) => void }) {
  ...
  return <article className="history-item">
    ...
    <div className="history-main"><h3>{session.taskName}</h3><div className="history-meta"><span>{session.category}</span><span>{start.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span><span>{session.status === "completed" ? "按计划完成" : "提前结束"}</span></div></div>
